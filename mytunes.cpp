...
void MyTunes::executeCMDFOLLOW(Command cmd){
    ...
    //add -l gjones "mysongs" 6
	     ...
	     if(cmd.hasToken("-u") && cmd.hasToken("-p") && cmd.hasToken("-f")) {
			 if (cmd.getToken("-f") == "stop") {
				 executeCeaseFollowing(cmd);
			 }
			 else {
				 executeFollowPlaylist(cmd);
				 }
		 }
		 else {
			 view.printOutput("INVALID FOLLOW SYNTAX");
		 }
}

//does a lot of validity checking, then assigns one playlist to follow another
void MyTunes::executeFollowPlaylist(Command cmd){
	string user_id = cmd.getToken("-u");
	string observerName = cmd.getToken("-p");
	string subjectUserID = cmd.getToken("-f");
	
	User* user = users.findByID(user_id);
	if(user == NULL) return;
	Playlist * playlist = user->findPlaylist(observerName);
	if(playlist == NULL) {
		view.printOutput("NO SUCH PLAYLIST.");
		return;
	}
	User * followedUser = users.findByID(subjectUserID);
	if(followedUser == NULL) return;
	else if (followedUser == user) {
		view.printOutput("CANNOT FOLLOW OWN PLAYLIST.");
		return;
	}
	Playlist * subjectPlaylist = followedUser->findPlaylist(observerName);
	if(subjectPlaylist == NULL) {
		view.printOutput("TARGET USER HAS NO PLAYLIST BY THAT NAME TO FOLLOW.");
		return;
	}
	
	//subjectPlaylist->attach(*playlist);
	if (subjectPlaylist->isFollowing() == playlist) {
		view.printOutput("CANNOT FOLLOW THAT PLAYLIST: IT FOLLOWS YOUR PLAYLIST.");
		return;
	}
	playlist->follows(subjectPlaylist);
	view.printOutput("SUCCESSFUL FOLLOW.");
}

//doesn't need to do a lot of validity checking, just tells a playlist to stop observing
void MyTunes::executeCeaseFollowing(Command cmd){
	string user_id = cmd.getToken("-u");
	string observerName = cmd.getToken("-p");
	...
	view.printOutput("NO LONGER FOLLOWING");
}